...
export default function CategoryPageIndex () {
    ...
    useEffect(() => {
        async function getProducts() {
            try {
                const response = await axios.get(`${process.env.REACT_APP_BACKEND_API_URL}/products/categories/${categoryId}`, config);
                const categoryPromisse = await axios.get(`${process.env.REACT_APP_BACKEND_API_URL}/categories`, config);
                setProductList(response.data);

                for (let i = 0; i < categoryPromisse.data.length; i++) {
                    if (categoryPromisse.data[i].id === Number(categoryId)) {
                        setCategoryName(categoryPromisse.data[i].name);
                    }
                }
            } catch (error) {
                console.log(error);
                ...
                <List>
                    {
                        ...
                        ?
                        <span>{"Não temos produtos desta categoria no estoque! :("}</span>
                        :
